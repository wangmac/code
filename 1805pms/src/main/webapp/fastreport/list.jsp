...
<table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${reports }" var="report">
			...
			<td align="center">${report.title}</td>
			<td align="center">${report.content}</td>
			<td align="center">${report.ctime}</td>
			<td align="center">${report.cman}</td>
			<td align="center">
			<a href="reportget.do?id=${report.id }" class="pn-opt">修改</a>
			<a href="reportdelete.do?id=${report.id }" class="pn-opt">删除</a>
			</td>
		</tr>
		</c:forEach>
	</tbody>
...
<div class="page pb15" style="float:right;">
	<span class="r inb_a page_b">
		...
		<a href="reportlist.do?currentPage=1">首页</a>
		<c:if test="${requestScope.currentPage-1>0 }">
			<a href="reportlist.do?currentPage=${requestScope.currentPage-1 }">上一页</a>
		</c:if>
		<c:if test="${requestScope.currentPage+1<=requestScope.pageCount }">
			<a href="reportlist.do?currentPage=${requestScope.currentPage+1 }">下一页</a>
		</c:if>
		<a href="reportlist.do?currentPage=${requestScope.pageCount }">尾页</a>
	</span>
</div>
